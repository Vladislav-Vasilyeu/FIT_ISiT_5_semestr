...
using (IRepository repository = Repository.Create("Celebrities"))
{
    ...
    }).AddEndpointFilter<NumberNotMinus>();
    ...
    }).AddEndpointFilter<ValidateMapPut>();
...
public class PhotoExistsFilter : IEndpointFilter
{
    public async ValueTask<object?> InvokeAsync(EndpointFilterInvocationContext context, EndpointFilterDelegate next)
    {
        ...
        var basePath = "D:\\Универ\\СТСР Смелов\\Готовые лабораторные\\laba5\\ASPA\\ASPA005_2\\Celebrities";
...
public class NumberNotMinus : IEndpointFilter
{
    public async ValueTask<object?> InvokeAsync(EndpointFilterInvocationContext context, EndpointFilterDelegate next)
    {
        ...
        if (id <= 0)
        {
            return Results.Problem(
                title: "Bad Request",
                detail: "Идентификатор id должен быть больше 0.",
                statusCode: 400
            );
        }
        return await next(context);
...
public class ValidateMapPut : IEndpointFilter
{
    public async ValueTask<object?> InvokeAsync(EndpointFilterInvocationContext context, EndpointFilterDelegate next)
    {
        int id = context.GetArgument<int>(0);
        ...
        if (celebrity.Id < 0 || string.IsNullOrEmpty(celebrity.Surname) || string.IsNullOrEmpty(celebrity.Firstname) || string.IsNullOrEmpty(celebrity.PhotoPath))
        {
            return Results.Problem(
                title: "Bad Request",
                detail: "Какое-то из полей null",
                statusCode: 400
            );
        }
        return await next(context);
