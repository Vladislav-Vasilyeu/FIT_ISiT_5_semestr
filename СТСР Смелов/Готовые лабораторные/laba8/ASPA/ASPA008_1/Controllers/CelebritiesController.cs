...
namespace ASPA008_1.Controllers
{
    ...
    public class CelebritiesController : Controller
    {
        ...
        private readonly string _customPhotosPath = @"D:\Универ\СТСР Смелов\Готовые лабораторные\laba8\ASPA\ASPA008_1\Photos";
        ...
        public IActionResult Celebrities(int id)
        {
            ...
            if (HttpContext.Items.TryGetValue(InfoAsyncActionFilter.Wikipedia, out var wikiRefs) && wikiRefs is Dictionary<string, string> wikiDict && wikiDict.Count > 0)
            {
                ViewBag.WikipediaReferences = wikiDict;
            }
            else
            {
                ViewBag.WikipediaReferences = null;
            }
