...
namespace Test_SL_LES
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            using (var context = new LesDbContext(options))
            {
                context.Database.EnsureCreated();

                // Добавляем тестовую знаменитость, чтобы кэш не был пустым
                var testCelebrity = new Celebrity
                {
                    FullName = "Test Celebrity",
                    Nationality = "US",
                    ReqPhotoPath = "test.jpg"
                };
                context.Celebrities.Add(testCelebrity);
                context.SaveChanges();
            }

            // Регистрация сервисов
            locator.RegisterInstance(typeof(DbContextOptions<LesDbContext>), options);
            locator.RegisterService(typeof(LesDbContext), typeof(LesDbContext), ServiceLifetime.Transient);
            locator.RegisterService(typeof(IRepository), typeof(LesRepository), ServiceLifetime.Transient);
            locator.RegisterService(typeof(ICelebrity), typeof(LesRepository), ServiceLifetime.Transient);
            locator.RegisterService(typeof(ILifeEvent), typeof(LesRepository), ServiceLifetime.Transient);
            locator.RegisterService(typeof(ICommon), typeof(LesRepository), ServiceLifetime.Transient);
            locator.RegisterService(typeof(ICashCelebrities), typeof(CashCelebrities), ServiceLifetime.Scoped, TimeSpan.FromSeconds(5));

            // Тестирование Transient-сервисов
            Console.WriteLine("Testing Transient Services:");
            using (var scope = locator.GetScopeFactory().CreateScope())
            {
                var repository = (IRepository)scope.GetService(typeof(IRepository));
            ...
            // Тестирование CashCelebrities (Scoped)
            Console.WriteLine("\nTesting CashCelebrities:");
            using (var scope1 = locator.GetScopeFactory().CreateScope())
            {
                var cache1 = (ICashCelebrities)scope1.GetService(typeof(ICashCelebrities));
                Console.WriteLine("Cache Scope 1 - First call:");
                foreach (var celeb in cache1.GetCachedCelebrities())
                {
                    Console.WriteLine($"ID={celeb.Id}, FullName={celeb.FullName}, Nationality={celeb.Nationality}");
                }

                Console.WriteLine("Cache Scope 1 - Second call (should be cached):");
                foreach (var celeb in cache1.GetCachedCelebrities())
                {
                    Console.WriteLine($"ID={celeb.Id}, FullName={celeb.FullName}, Nationality={celeb.Nationality}");
                }
            ...
            // Демонстрация нового scope
            using (var scope2 = locator.GetScopeFactory().CreateScope())
            {
                var cache2 = (ICashCelebrities)scope2.GetService(typeof(ICashCelebrities));
                Console.WriteLine("Cache Scope 2 - New scope (new cache):");
                foreach (var celeb in cache2.GetCachedCelebrities())
                {
                    Console.WriteLine($"ID={celeb.Id}, FullName={celeb.FullName}, Nationality={celeb.Nationality}");
                }
            }

            // Демонстрация кэширования по времени
            Console.WriteLine("\nTesting Cache Expiration:");
            using (var scope3 = locator.GetScopeFactory().CreateScope())
            {
                var cache3 = (ICashCelebrities)scope3.GetService(typeof(ICashCelebrities));
                Console.WriteLine("Cache Scope 3 - First call:");
                foreach (var celeb in cache3.GetCachedCelebrities())
                {
                    Console.WriteLine($"ID={celeb.Id}, FullName={celeb.FullName}, Nationality={celeb.Nationality}");
                }

                Console.WriteLine("Waiting 6 seconds to expire cache...");
                System.Threading.Thread.Sleep(6000);

                Console.WriteLine("Cache Scope 3 - After expiration:");
                foreach (var celeb in cache3.GetCachedCelebrities())
                {
                    Console.WriteLine($"ID={celeb.Id}, FullName={celeb.FullName}, Nationality={celeb.Nationality}");
                }
