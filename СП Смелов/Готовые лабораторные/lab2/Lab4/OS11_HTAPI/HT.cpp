...
namespace ht
{
		...
		const wchar_t* fileName)		
	{
		...
		htHandle->mutex = CreateMutexW(NULL, TRUE, HT_GLOBAL_MUTEX_NAME);
		...
		if (GetLastError() == ERROR_ALREADY_EXISTS)
		{
			...
			CloseHandle(hm);
			CloseHandle(hf);
			throw "HT already opened by another process";
		}
		...
		const wchar_t* fileName)
	{
		...
		htHandle->addr = lp;
		htHandle->lastSnaptime = time(NULL);
		htHandle->mutex = OpenMutexW(MUTEX_ALL_ACCESS, FALSE, HT_GLOBAL_MUTEX_NAME);
		if (htHandle->mutex == NULL)
		{
			UnmapViewOfFile(lp);
			CloseHandle(hm);
		...
		DWORD waitResult = WaitForSingleObject(htHandle->mutex, INFINITE);
