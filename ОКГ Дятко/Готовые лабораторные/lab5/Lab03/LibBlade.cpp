...
{
	// Вычисляем матрицу перехода мировые -> оконные
	CMatrix M = SpaceToWindow(RS, RW);

	auto ToWnd = [&](const CMatrix& p)->CPoint {
		CMatrix pt(3);
		pt(0) = p(0); pt(1) = p(1); pt(2) = 1;
		CMatrix r = M * pt;
		int rx = (int)round(r(0));
		int ry = (int)round(r(1));
		return CPoint(rx, ry);
	};
	...
	// Режим MM_TEXT (чтобы мы рисовали в оконных координатах)
	dc.SetMapMode(MM_TEXT);

	// рисуем от преобразованных координат
	CMatrix O(3);
	O(0) = 0; O(1) = 0; O(2) = 1;
	dc.MoveTo(ToWnd(O));
	dc.LineTo(ToWnd(FTCoords));
	DrawTriangle(FTCoords, STCoords, dc, true, M);
	DrawTriangle(FBCoords, SBCoords, dc, true, M);
	DrawTriangle(FTHCoords, STHCoords, dc, false, M);
	DrawTriangle(FBHCoords, SBHCoords, dc, false, M);

	dc.SelectObject(&VBrush);

	// MainPoint — прямоугольник в мировых координатах, преобразуем его углы
	CMatrix mp1(3); mp1(0) = MainPoint.left; mp1(1) = MainPoint.top; mp1(2) = 1;
	CMatrix mp2(3); mp2(0) = MainPoint.right; mp2(1) = MainPoint.bottom; mp2(2) = 1;
	CPoint p1 = ToWnd(mp1);
	CPoint p2 = ToWnd(mp2);
	CRect ellRect(p1, p2);
	dc.Ellipse(ellRect);
...
void CBlade::DrawTriangle(const CMatrix& FTCoords, const CMatrix& STCoords, CDC& dc, bool color, const CMatrix& M)
{
	auto ToWnd = [&](const CMatrix& p)->CPoint {
		CMatrix pt(3);
		pt(0) = p(0); pt(1) = p(1); pt(2) = 1;
	...
	if (color) {
		...
		CPoint points[3];
		points[0] = ToWnd(origin); // origin
		points[1] = ToWnd(FTCoords);
		points[2] = ToWnd(STCoords);
