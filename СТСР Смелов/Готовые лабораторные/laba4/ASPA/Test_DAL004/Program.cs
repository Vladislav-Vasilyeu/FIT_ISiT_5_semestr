    ...
    internal class Program
    {
        private static void Main(string[] args)
        {
            ...
            using (IRepository repository = Repository.Create("Celebrities"))
            {
                ...
                if (testupd1 != null)
                    if (repository.updCelebrityById((int)testupd1, new Celebrity(0, "Updated1", "Updated1", "Photo/Updated1.jpg"))is not null) Console.WriteLine($" update {testupd1} "); 
                    else Console.WriteLine($" update {testupd1} error");
                if (testupd2 != null)
                    if (repository.updCelebrityById((int)testupd2, new Celebrity(0, "Updated2", "Updated2", "Photo/Updated2.jpg")) is not null) Console.WriteLine($" update {testupd2} ");
                    else Console.WriteLine($" update {testupd2} error");
                if (repository.updCelebrityById(1000, new Celebrity(0, "Updated1000", "Updated1000", "Photo/Updated1000.jpg")) is not null) Console.WriteLine($" update {1000} ");
