...
namespace DAL_Celebrity_MSSQL
{
    public class Init
    {
        static string connstring = @"Server=VladPC;Database=Life Events of  Celebrities;Trusted_Connection=True;TrustServerCertificate=True;";
        public Init() { }
        ...
        public static void Execute(bool delete = true, bool create = true)
        {
            ...
            if (delete) 
                context.Database.EnsureDeleted();   // если есть БД, то она удаляется
            if (create) 
                context.Database.EnsureCreated();   // если нет  БД, то содается 
