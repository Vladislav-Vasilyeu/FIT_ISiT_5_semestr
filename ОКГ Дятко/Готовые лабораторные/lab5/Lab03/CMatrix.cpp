...
double CMatrix::operator()(int i, int j) const
{
	if ((i > n_rows - 1) || (j > n_cols - 1))     //  проверка выхода за диапазон
	{
		TCHAR* error = _T("CMatrix::operator(int,int) const: выход индекса за границу диапазона ");
		MessageBox(NULL, error, _T("Ошибка"), MB_ICONSTOP);
		exit(1);
	}
	return array[i][j];
...
double CMatrix::operator()(int i) const
{
	if (n_cols > 1)     //  Число столбцов больше одного
	{
		char* error = "CMatrix::operator(int) const: объект не вектор - число столбцов больше 1 ";
		MessageBox(NULL, (LPCWSTR)error, L"Ошибка", MB_ICONSTOP);
		exit(1);
	}
	if (i > n_rows - 1)     //  проверка выхода за диапазон
	{
		TCHAR* error = TEXT("CMatrix::operator(int) const: выход индекса за границу диапазона ");
		MessageBox(NULL, error, TEXT("Ошибка"), MB_ICONSTOP);
		exit(1);
	}
	return array[i][0];
...
//---------------------------------------------------------------------------------
CMatrix CMatrix::GetRow(int k) const
...
//---------------------------------------------------------------------------------
CMatrix CMatrix::GetRow(int k, int n, int m) const
...
//---------------------------------------------------------------------------------
CMatrix CMatrix::GetCol(int k) const
...
//---------------------------------------------------------------------------------
CMatrix CMatrix::GetCol(int k, int n, int m) const
