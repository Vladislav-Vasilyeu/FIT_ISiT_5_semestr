...
string createMessage(int number) {
	char message[50];
	sprintf_s(message, sizeof(message), "Hello from Client %d", number);
...
int main()
{
    ...
    try
    {
        ...
        for (int i = 0; i < messageCount; i++) {
            ...
            if (send(cC, (char*)&messageLen, sizeof(messageLen), 0) == SOCKET_ERROR)
                throw SetErrorMsgText("send length failed:", WSAGetLastError());
            if (send(cC, message.c_str(), messageLen, 0) == SOCKET_ERROR)
                throw SetErrorMsgText("send message failed:", WSAGetLastError());

            cout << "Sent: " << message << endl;

            int receivedLen;
            int bytesReceived = recv(cC, (char*)&receivedLen, sizeof(receivedLen), 0);
            if (bytesReceived == SOCKET_ERROR)
                throw SetErrorMsgText("recv length failed:", WSAGetLastError());

            char buffer[1024];
            bytesReceived = recv(cC, buffer, receivedLen, 0);
            if (bytesReceived == SOCKET_ERROR)
        ...
        int endSignal = 0;
        send(cC, (char*)&endSignal, sizeof(endSignal), 0);
